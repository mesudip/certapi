...
def get_crt(
    ...
    def _do_request(url, data=None, err_msg="Error", depth=0):
        ...
        try:
            resp = urlopen(
                Request(
                    url, data=req_data, headers={"Content-Type": "application/jose+json", "User-Agent": "acme-tiny"}
                )
            )
            ...
            if "nonce" not in url:
                ...
                print(url)
                if code != 200:
                    print(json.dumps({x[0]: x[1] for x in headers.items()}, indent=2))
                print(resp_data)
                print("-" * 60)
        except IOError as e:
        ...
        if depth < 100 and code == 400 and resp_data["type"] == "":
    ...
    def _send_signed_request(url, payload, err_msg, depth=0):
        ...
        data = {"protected": protected64, "payload": payload64, "signature": _b64(out)}
        print(url)
        print("data.payload", "" if payload is None else json.dumps(payload, indent=2))
        print("request", json.dumps(data, indent=2))
        try:
            return _do_request(url, data, err_msg=err_msg, depth=depth)
    ...
    if contact is not None:
        account, _, _ = _send_signed_request(
            acct_headers["Location"], {"contact": contact}, "Error updating contact details"
        )
